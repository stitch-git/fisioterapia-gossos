...
export default function IOSInstallBanner() {
  ...
  const deviceType = browserInfo.isIPad 
    ? t('iosInstallBanner.devices.iPad')
    : t('iosInstallBanner.devices.iPhone')
      ...
      <div className="px-4 py-3 sm:px-6 sm:py-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between space-y-3 sm:space-y-0">
          ...
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1 min-w-0">
              <h3 className="text-sm sm:text-base font-semibold text-white">
                {t('iosInstallBanner.title', { device: deviceType })}
              </h3>
              <p className="text-xs sm:text-sm text-blue-100 mt-1 leading-tight">
                {t('iosInstallBanner.subtitle')}
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3">
            ...
            >
              {t('iosInstallBanner.showHowToInstall')}
            </button>
          </div>
        </div>
        ...
        <div className="mt-3 sm:hidden">
          <div className="flex items-center justify-center space-x-2 text-blue-100">
            ...
            <span className="text-xs">{t('iosInstallBanner.tap')}</span>
            <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd" />
            </svg>
            <span className="text-xs">{t('iosInstallBanner.addToHome')}</span>
          </div>
        </div>
      </div>
