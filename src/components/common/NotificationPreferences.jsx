...
export default function NotificationPreferences({
  ...
  const handleEmailChange = (e) => {
    ...
    // If both are unchecked after this change, prevent it
    if (!isChecked && !emailEnabled) {
      return // Don't allow unchecking if it would leave both unchecked
    }
  ...
  return (
    <div className={className}>
      <fieldset>
        ...
        {/* Container responsive para opciones */}
        <div className="space-y-4">
          
          {/* Opción Email - RESPONSIVE */}
              ...
              <label htmlFor="email-notifications" className="flex items-start sm:items-center cursor-pointer">
                <div className="flex flex-col sm:flex-row sm:items-center">
                  ...
                  <div className="flex items-center mt-1 sm:mt-0 sm:ml-2">
                    ...
                    <span className="text-xs text-blue-600 ml-1 hidden sm:inline">Email</span>
                  </div>
                </div>
              </label>
            </div>
...
export const validateNotificationPreferences = (email, sms = false, push = false) => {
  // Email siempre debe estar en true (obligatorio)
  if (!email) {
    return {
      valid: false,
      message: 'Las notificaciones por email son obligatorias'
    }
  }
